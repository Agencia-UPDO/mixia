...
if (!function_exists('mixia_bot_admin_menu')) :
...
function mixia_bot_save_settings()
{
    ...
    $backend_url    = esc_url_raw(isset($_POST['mixia_bot_backend_url']) ? $_POST['mixia_bot_backend_url'] : '');
    $admin_token    = sanitize_text_field(isset($_POST['mixia_bot_admin_token']) ? $_POST['mixia_bot_admin_token'] : '');
    ...
    $wc_secret     = sanitize_text_field(isset($_POST['mixia_bot_wc_secret']) ? $_POST['mixia_bot_wc_secret'] : '');
    ...
    if ($has_keys && $backend_url && $admin_token) {
        $payload = array();
        if ($anthropic_key) $payload['anthropic_api_key'] = $anthropic_key;
        if ($wc_key)        $payload['woocommerce_key']   = $wc_key;
        if ($wc_secret)     $payload['woocommerce_secret'] = $wc_secret;
        ...
        if (is_wp_error($response)) {
            ...
            $msg    = ($code === 200)
                ? 'Configuracoes salvas e chaves atualizadas no backend!'
                : 'Configuracoes locais salvas, mas o backend retornou erro ' . $code . '.';
        }
    } else {
        $notice = 'success';
...
?>
    <div class="wrap">
        <h1>Mixia Bot - Configuracoes</h1>

        <?php if ($notice === 'success') : ?>
            <div class="notice notice-success is-dismissible">
                <p><?php echo esc_html($msg ? $msg : 'Configuracoes salvas!'); ?></p>
            </div>
        ...
        <form method="POST" action="<?php echo admin_url('admin-post.php'); ?>">
                ...
                <tr>
                    ...
                    <td>
                        <input type="url" id="mixia_bot_backend_url" name="mixia_bot_backend_url"
                            value="<?php echo esc_attr($backend_url); ?>" class="regular-text"
                ...
                <tr>
                    ...
                    <td>
                        <input type="password" id="mixia_bot_admin_token" name="mixia_bot_admin_token"
                            value="<?php echo esc_attr($admin_token); ?>" class="regular-text">
                    </td>
                </tr>
            </table>
                ...
                <tr>
                    ...
                    <td>
                        <input type="password" id="mixia_bot_anthropic_key" name="mixia_bot_anthropic_key"
                            value="" class="regular-text" placeholder="<?php echo $anthropic_set ? 'ja configurada' : 'sk-ant-...'; ?>">
                ...
                <tr>
                    ...
                    <td>
                        <input type="password" id="mixia_bot_wc_key" name="mixia_bot_wc_key"
                            value="" class="regular-text" placeholder="<?php echo $wc_key_set ? 'ja configurada' : 'ck_...'; ?>">
                ...
                <tr>
                    ...
                    <td>
                        <input type="password" id="mixia_bot_wc_secret" name="mixia_bot_wc_secret"
                            value="" class="regular-text" placeholder="<?php echo $wc_secret_set ? 'ja configurada' : 'cs_...'; ?>">
            ...
            <table class="form-table">
                <tr>
                    <th><label for="mixia_bot_widget_title">Titulo do Chat</label></th>
                    <td>
